...
module tb_top();
    ...
    task btn_press;
         ...
         begin
            ...
            // 1. make noise (0.55ms)
            btn[btn_index] = 1; #100; // 0.1 ms High
            btn[btn_index] = 0; #200; // 0.2 ms Low
            btn[btn_index] = 1; #150; // 0.15 ms High
            btn[btn_index] = 0; #100; // 0.1 ms Low
            // 2. 안정 구간 11ms 유지 --> 이 구간이 지나야 clean_btn이 1이 된다.
            btn[btn_index] = 1;
            #11_000_000; // 11ms 유지 --> 이 구간이 지나야 clean_btn이 1이 된다.
            // 3. btn을 뗀다(10ms 이상)
            btn[btn_index] = 0;
            #11_000_000;
    ...
    initial begin
        ...
        #50_000_000; // 50ms 대기 (스탑워치 ms가 올라가는지 확인)

        $display("\n--- 3. Stopwatch: PAUSE ---");
        btn_press(2); // btn[2] 누름 (정지)

        $display("\n--- 4. ChangeMode: Stopwatch -> IDLE ---");

        #100_000_000; // 100ms 대기 (멈춰있는지 확인)

        $display("\n--- 5. IDLE: circular move ---");
        #50_000_000; // 50ms 대기

        $display("\n--- 6. Change Mode: STOPWATCH -> CLOCK ---");
        btn_press(0); // btn[0] 누름
